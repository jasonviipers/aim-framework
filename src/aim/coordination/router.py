...
class CapabilityRouter:
    ...
    async def route_request(self, request: Request) -> List[str]:
        ...
        if cache_key in self.routing_cache:
            ...
            if self._validate_route(cached_route):
                self.logger.debug(f"Using cached route for request {request.request_id}")
                return cached_route
        ...
        self.logger.info(f"Routed request {request.request_id} to agents: {routing_path}")
        return routing_path
    ...
    async def _select_optimal_path(self, request: Request, agents: List[Agent]) -> List[str]:
        """
        ...
        for agent in agents:
            ...
            # Combined score
            score = confidence * 0.5 + (1 - load_factor) * 0.3 + performance_factor * 0.2
