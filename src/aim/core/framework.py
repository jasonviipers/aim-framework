...
class AIMFramework:
    ...
    async def process_request(self, request: Request) -> Response:
        ...
        try:
            self.logger.info(f"Processing request {request.request_id} from user {request.user_id}")
            ...
            response = await self.collaborator.process_with_collaboration(request, agent_path)
            ...
            self.logger.info(f"Completed request {request.request_id} in {processing_time:.3f}s")
    ...
    async def _update_context_thread(self, request: Request, response: Response) -> None:
        """
        ...
        try:
            ...
            if response.context_updates:
                self.context_manager.update_thread_context(
                    request.context_thread_id,
                    response.context_updates
                )
                
        except Exception as e:
            self.logger.warning(f"Failed to update context thread {request.context_thread_id}: {e}")
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                if cleaned_threads > 0:
                    self.logger.info(f"Cleaned up {cleaned_threads} expired context threads")
