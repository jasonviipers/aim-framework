...
class ContextThread:
    ...
    def _validate(self) -> None:
        ...
        if self.max_interactions <= 0:
            raise ValidationError("max_interactions", self.max_interactions, "Max interactions must be positive")
    ...
    def get_interactions_by_agent(self, agent_id: str) -> List[ContextInteraction]:
        ...
        """
        return [interaction for interaction in self.interactions if interaction.agent_id == agent_id]
...
class ContextManager:
    ...
    def __init__(self, max_threads_per_user: int = 10, cleanup_interval: float = 3600.0):
        """
    ...
    def get_user_threads(self, user_id: str) -> List[ContextThread]:
        ...
        return [self.threads[thread_id] for thread_id in self.user_threads[user_id] if thread_id in self.threads]
    ...
    def add_interaction(
        ...
        """
        thread = self.get_thread(thread_id)
        return thread.add_interaction(request_content, response_content, agent_id, confidence, metadata)
    ...
    def _prune_user_threads(self, user_id: str) -> None:
        ...
        user_threads = [(thread_id, self.threads[thread_id]) for thread_id in user_thread_ids if thread_id in self.threads]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        total_interactions = sum(len(thread.interactions) for thread in self.threads.values())
        ...
        return {
            ...
            "average_interactions_per_thread": total_interactions / len(self.threads) if self.threads else 0,
