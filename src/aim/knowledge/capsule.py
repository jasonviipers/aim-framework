...
class KnowledgeCapsule:
    ...
    def calculate_relevance_score(self, query_tags: Set[str]) -> float:
        ...
        weighted_score = jaccard_similarity * self.confidence * (0.5 + 0.5 * self.success_rate)
    ...
    def merge_with(self, other: "KnowledgeCapsule") -> "KnowledgeCapsule":
        ...
        if total_usage > 0:
            merged_confidence = (
                (self.confidence * self.usage_count + other.confidence * other.usage_count)
                / total_usage
            )
        else:
        ...
        source_agent = self.source_agent if self.created_at > other.created_at else other.source_agent
